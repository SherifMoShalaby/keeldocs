...
**As of 2026-08-07.** Published: **`keeldocs@0.4.3` on npm** — `latest` — built by
...
registry, not from the workflow log: attestation naming `refs/tags/v0.4.3`, and
...
runs before it and fixed the same day (§4 item 6) · 201 unit tests · 40 extractor goldens · 98 harness checks ·
...
capabilities feed 8 document recipes. The engine has 201 unit tests, 40
...
that ships to users is now stable `0.4.3` on npm, so promoting the v0.1 tag is a
...
agent skills · 201 unit tests · 40 byte-compared extractor goldens · ~25
