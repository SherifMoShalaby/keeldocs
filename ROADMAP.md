...
**As of 2026-08-08.** Published: **`keeldocs@0.5.0` on npm** — `latest` — built by
...
registry, not from the workflow log: attestation naming `refs/tags/v0.5.0`, and
...
that ships to users is now stable `0.5.0` on npm, so promoting the v0.1 tag is a
...
| Public repos with committed anchors | ≥50 | **Unknown — the instrument is blind, and that is now its own verdict** — `scripts/dev/adoption.py` returns `BLIND (self_not_indexed)`. Not "0": this row used to print one, and a blind index and no adopters produce the same number. Series in `scripts/dev/adoption-log.jsonl` |
