...
**R4 derived PostgREST surface — SHIPPED (owner decision 2026-08-01).**
`sql-replay` introspects routines alongside tables and enums (seed and
extension-stub routines are excluded by a signature+body-digest baseline,
not by schema, because an unqualified `CREATE FUNCTION` lands in `public`),
and `http-endpoints/supabase-postgrest` derives the REST API from that
catalog through a declared `${facts:db-schema}` read — one replay, two
capabilities. Exposure follows `supabase/config.toml [api]`; a `Database
functions` section joins the ERD recipe. E9 round 4: route claims 5 → 0 on
the field repo, denominator 210 → 438, still 100% documented. Views,
materialized views and `PUT` are deliberately unclaimed and reported as
gaps rather than guessed.

**Recipe migration — NEW debt, opened by R4.** A recipe that GROWS a section
does not retrofit it into docs generated by an earlier version: `init` never
overwrites an existing file and `sync` only regenerates regions that already
