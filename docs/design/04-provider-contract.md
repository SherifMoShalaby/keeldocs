...
Code providers run as short-lived subprocesses: JSON facts on stdout, JSON errors on stderr; the *engine* writes all artifacts. Policy is declared in the manifest and shown at grant time: network deny-all (sole exception class `network:db`, one socket to the runtime-resolved host — never an address from provider code); read-only FS scoped to declared globs minus the security exclusion set (`.env*` values, key material, cloud credential dirs); writes only to per-run scratch; no undeclared child processes (`exec: ["git"]` must be declared); two accepted symbol shapes for `module-graph` — flat `symbols`, or `symbolFiles` grouping them under their file so `path` and `package` are written once each (D11); an optional `nameless` the engine derives from `(name, sigs)` when absent (D8); an optional `incremental: per-file` declaration, under which the engine hands back the provider's own previous per-file parses (keyed by content digest) so an edit re-parses only what changed — a performance input that must never change the answer; an output cap of `clamp(6 × declared input bytes, 5MB, 256MB)` — proportional since D2, because a constant cannot say what "runaway" means without knowing what the provider was handed; enforcement tiered by OS (Landlock/seccomp, Seatbelt, best-effort elsewhere) with the deterministic suite CI-run under a network-denied sandbox as a permanent regression test of the offline claim.
