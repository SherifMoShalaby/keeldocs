...
## 7. Recipe migration — when a recipe GROWS a section (v0.3)

A recipe is versioned (`recipe=erd@1`) but it also evolves *within* a version: the R4 routine facts added a `## Database functions` block to the erd recipe without changing what any existing block means. Documents generated earlier had no way to receive it — `init` never overwrites an existing file, `sync` only regenerates regions that already exist — so the section was reachable only by deleting the document and re-generating it, which is precisely the act that destroys prose written into slots or below the human-notes line. `keeldocs sync --upgrade` closes that hole, under five rules that are all refusals.

A **section** is a `## ` heading and everything up to the next `## ` or the human-notes marker, identified by the marker ids inside it and never by its heading text — so a heading a human renamed cannot resurrect a duplicate. A section is **missing** only when *none* of its ids appear anywhere in the document, which is what makes the pass idempotent. Insertion is **pure addition**: no existing region is rewritten, nothing is reordered, and the inserted body carries hashes computed from current facts, so it is born clean. Anchoring is **semantic** — a proposal records the marker id it must follow, resolved against the document as it stands at apply time — which lets consecutive new sections chain onto each other in recipe order and makes an out-of-order apply fail loudly instead of landing in the wrong place. A document whose **root anchor is absent** is refused outright: a same-path file somebody else wrote is not ours to edit.

A document older than its recipe is not stale, not lying, and not drift. `check` therefore reports `data.upgrades` for discoverability and leaves the exit code alone; `--upgrade` is a mode a human asks for, never something `--apply-all` does by accident. A section a human does not want is held by the ordinary rejection journal, with no new machinery.
